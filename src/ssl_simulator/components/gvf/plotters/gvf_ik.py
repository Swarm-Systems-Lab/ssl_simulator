...
class PlotterGvfIk(PlotterVF):
    ...
    def draw(
        ...
        **kwargs,
        ...
        if draw_field:
            ...
            self.cond_map = self.ax.pcolormesh(
                self.cond_xx,
                self.cond_yy,
                self.cond_flags,
                shading="auto",
    ...
    def _compute_vector_field(self):
        ...
        pd_dot_x = np.where(cond_flags, alpha * ut_hat_x + un_x, self.speed * un_x / un_norm)
        pd_dot_y = np.where(cond_flags, alpha * ut_hat_y + un_y, self.speed * un_y / un_norm)
