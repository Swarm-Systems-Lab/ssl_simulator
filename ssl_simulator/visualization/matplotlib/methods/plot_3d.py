__all__ = [
    "plot_so3_heading_traj",
    "plot_so3_omega_traj",
    "plot_so3_attitude_vectors"
]
...
from ssl_simulator.visualization import initialize_plot
...
def plot_3d_sphere_wf(ax, radius, projections=False, surface=False):
    ...
    rstride = int(round(15 / (180 / (N_lat - 1))))  # along latitude
    cstride = int(round(15 / (360 / (N_lon - 1))))  # along longitude

    # Wireframe sphere and surface
    wf = ax.plot_wireframe(x, y, z, rstride=rstride, cstride=cstride, 
                           linewidth=0.6, alpha=0.2, color="k", zorder=0)
    artists["sphere_wireframe"] = wf

    if surface:
        # Surface plot with low alpha so wireframe and lines are visible
        sf = ax.plot_surface(x, y, z, 
                             color="lightgrey", alpha=0.1, edgecolor="none", zorder=-1)
    ...
    kw_lines = dict(linewidth=2.5, color="grey", alpha=1)

    mid_len = x_lon0.shape[0]//2
    m1, = ax.plot(x_lon0[0:mid_len], y_lon0[0:mid_len], z_lon0[0:mid_len], 
            **kw_lines, label="Lon 0º")
    m2, = ax.plot(x_lon90[0:mid_len], y_lon90[0:mid_len], z_lon90[0:mid_len], 
            **kw_lines, label="Lon 90º")
    m3, = ax.plot(x_lon0[mid_len:], y_lon0[mid_len:], z_lon0[mid_len:], 
            **kw_lines, linestyle="--")
    m4, = ax.plot(x_lon90[mid_len:], y_lon90[mid_len:], z_lon90[mid_len:], 
            **kw_lines, linestyle="--")
    ...
    if projections:
        ...
        ax.contour(
            x, y, z, levels=levels,
            alpha=0.2, linewidths=0.7, colors=["k"],
            zdir="z", offset=zlim[0]*1.04,
        )
        ax.contour(
            x, y, z, levels=levels,
            alpha=0.2, linewidths=0.7, colors=["k"],
            zdir="y", offset=ylim[1]*1.04,
        )
        ax.contour(
            x, y, z, levels=levels,
            alpha=0.2, linewidths=0.7, colors=["k"],
            zdir="x", offset=xlim[0]*1.04,    
        )
...
def plot_so3_attitude_vectors(R_data, p_data=None, ax=None, arr_len=1.0, l_list=None, sphere=True, quiver_thickness=2):
    """
    ...
    l_len = len(l_list)
    quivers_array = np.empty((l_len, n_agents, 3), dtype=object)
    scatter_array = np.empty((l_len, n_agents), dtype=object)

    for li, l in enumerate(l_list):
        for n in range(n_agents):
            x, y, z = p_data[l, n]  # unpack coordinates
            # Scatter
            ...
            for i in range(3):
                q = main_ax.quiver(
                    x, y, z,
                    R_data[l, n, 0, i], R_data[l, n, 1, i], R_data[l, n, 2, i],
                    color=ax_cols[i], length=0.999*arr_len, normalize=True, alpha=1,
...
def plot_so3_heading_traj(R_data, ax=None, lim=1.6):
    ...
    if R_data.shape[1] > 1:
        for n in range(R_data.shape[1]):
            ...
            if R_data.shape[0] > 1:
                main_ax.plot(u[-1,n,0], u[-1,n,1], "og", zdir="z", zs=-lim, markersize=2, alpha=1)
                main_ax.plot(u[-1,n,0], u[-1,n,2], "og", zdir="y", zs= lim, markersize=2, alpha=1)
                main_ax.plot(u[-1,n,1], u[-1,n,2], "og", zdir="x", zs=-lim, markersize=2, alpha=1)
...
def plot_so3_omega_traj(R_data, ax=None):
    ...
    for n in range(R_data.shape[1]):
        ...
        main_ax.plot(omega[:,n,0], omega[:,n,1], ".r", zdir="z", zs=zlim[0]*1.04, markersize=0.5, alpha=0.7)
        main_ax.plot(omega[:,n,0], omega[:,n,2], ".r", zdir="y", zs=ylim[1]*1.04, markersize=0.5, alpha=0.7)
        main_ax.plot(omega[:,n,1], omega[:,n,2], ".r", zdir="x", zs=xlim[0]*1.04, markersize=0.5, alpha=0.7)
