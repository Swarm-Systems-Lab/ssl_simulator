...
def construct_attitude_basis(heading, gravity):
    ...
    gravity = check_and_parse_dimensions(
        gravity, (None, 3), "gravity", fill_values=heading.shape[0]
    )
...
def so3_log_map(R, n=5, eps=1e-8):  # TODO: test and revise
    ...
    if np.any(large_mask):
        ...
        diag = np.stack(
            [R_flat[large_mask, 0, 0], R_flat[large_mask, 1, 1], R_flat[large_mask, 2, 2]], axis=1
        )
    ...
    if np.any(mid_mask):
        ...
        for i in idx:
            ...
            for j in range(n):
                ...
                # θ_i / (2 sin θ_i)
                coeff = 1 / 2 if np.abs(np.sin(theta_i)) < eps else theta_i / (2 * np.sin(theta_i))
...
def so3_rotate_with_step(R, omega_hat, step=np.pi / 6):
    ...
    if R.shape[:-2] != omega_hat.shape[:-2]:
        if R.shape[:-2] == (1,):
            ...
            raise ValueError(
                f"Incompatible batch shapes: R {R.shape[:-2]} vs omega_hat {omega_hat.shape[:-2]}"
            )
    ...
    omega_vec = np.stack(
        [omega_hat_flat[:, 2, 1], omega_hat_flat[:, 0, 2], omega_hat_flat[:, 1, 0]], axis=1
    )
    theta = np.linalg.norm(omega_vec, axis=1)

    # Normalize angles > 2*pi
    mask_large = theta > 2 * np.pi
    omega_hat_flat[mask_large] *= (theta[mask_large] % (2 * np.pi) / theta[mask_large])[
        :, None, None
    ]
    ...
    if np.any(mask_single):
        R_rot_flat[mask_single] = np.matmul(
            R_rot_flat[mask_single], so3_exp_map(omega_hat_flat[mask_single])
        )
...
def so3_right_jacobian(phi):
    ...
    if phi.ndim == 1:
        ...
        J = eye3 - A * phi_hat + B * phi_hat @ phi_hat
        return J
...
def so3_right_jacobian_inv(phi):
    ...
    if phi.ndim == 1:
        ...
        J_inv = eye3 + A * phi_hat + B * np.matmul(phi_hat, phi_hat)
        return J_inv
