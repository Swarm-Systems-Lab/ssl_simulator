...
def batman_distrib(N, rc0, lims, scale=1):
    ...
    * lims = [xlim, ylim]: width and length of the distribution.
    ...
    while len(x_filt) < N:
        ...
        def eq1(x, y):
            return (
                (x / 7) ** 2 * np.sqrt(abs(abs(x) - 3) / (abs(x) - 3))
                + (y / 3) ** 2 * np.sqrt(abs(y + 3 / 7 * np.sqrt(33)) / (y + 3 / 7 * np.sqrt(33)))
                - 1
            )
        ...
        def eq3(x, y):
            return (
                9 * np.sqrt(abs((abs(x) - 1) * (abs(x) - 0.75)) / ((1 - abs(x)) * (abs(x) - 0.75)))
                - 8 * abs(x)
                - y
            )
        ...
        def eq4(x, y):
            return (
                ...
                + 0.75
                * np.sqrt(
                    abs((abs(x) - 0.75) * (abs(x) - 0.5)) / ((0.75 - abs(x)) * (abs(x) - 0.5))
                )
                - y
            )

        def eq5(x, y):
            return 2.25 * np.sqrt(abs((x - 0.5) * (x + 0.5)) / ((0.5 - x) * (0.5 + x))) - y

        def eq6(x, y):
            return (
                6 * np.sqrt(10) / 7
                + (1.5 - 0.5 * abs(x)) * np.sqrt(abs(abs(x) - 1) / (abs(x) - 1))
